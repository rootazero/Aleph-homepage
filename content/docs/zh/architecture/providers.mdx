---
title: "提供商"
description: "LLM 提供商系统、三层协议适配器、故障转移与断路器、20+ 预设支持。"
---

Aleph 的提供商架构将**供应商**与**协议**解耦。Aleph 不是为每个提供商做一次性集成，而是按协议族对提供商进行分组。一个 OpenAI 协议适配器处理 OpenAI、DeepSeek、Moonshot 和任何其他 OpenAI 兼容 API。添加新供应商通常只需要一个预设条目 —— 无需新代码。
...
1. **Preset lookup** — `create_provider("deepseek", config)` 检查 `presets::get_preset("deepseek")`
2. **Preset defaults applied** — 如果缺失则自动填充 `base_url`、`protocol`、`color`
3. **Protocol resolution** — `config.protocol()` 确定适配器（默认为 `"openai"`）
4. **Special cases** — `ollama` 和 `mock` 使用原生实现
5. **Adapter lookup** — `ProtocolRegistry::global().get(protocol_name)` 返回适配器
6. **Provider instantiation** — `HttpProvider::new(name, config, adapter)` 包装适配器
...
提供商是线程安全的（`Send + Sync`）并通过 `Arc<dyn AiProvider>` 共享。单个 `process()` 方法接受包含结构化 `UnifiedMessage` 历史的 `RequestPayload`，协议适配器将这些转换为原生 API 格式。

## 协议适配器

`ProtocolRegistry` 将协议名称映射到 `ProtocolAdapter` 实现。内置协议通过 `Lazy` 静态初始化在初始化时注册。
...
查找顺序：
1. **Dynamic protocols first** — 从 `~/.aleph/protocols/*.yaml` 加载
2. **Built-in protocols fallback** — 工厂函数按需实例化适配器
...
- `build_request(payload, config)` — 构造 HTTP 请求构建器（stream-first：始终设置 `stream: true`）
...
这些使用 `openai` 协议适配器，配合供应商特定的 `base_url`：
...
**注意：** `kimi-for-coding` 使用 `anthropic` 协议（不是 `openai`），专为 Claude Code 和 Cline 等 IDE/agent 工具使用场景设计。对于一般聊天，请使用 `moonshot` / `kimi` 预设。

## 提供商配置

每个提供商通过 `config.toml` 中的 `ProviderConfig` 配置：
...
`FailoverProvider` 包装多个提供商并自动在故障时路由。它实现相同的 `AiProvider` 特征，因此调用者看到的是一个统一的提供商。
...
**错误分类：**
- **Non-retryable**（立即故障转移）：`AuthenticationError`、`InvalidConfig`
- **Retryable**（带退避重试）：rate limits、timeouts、network errors
- **Rate limits**（特殊）：标记提供商不健康后触发立即故障转移

### 健康跟踪

`health.rs` 模块提供一个单独的 `ProviderHealth` 枚举，供认证配置文件系统使用：
...
冷却使用指数退避：基础 30 秒，每次失败翻倍，上限为 5 分钟。带有 `retry_after` 头部的速率限制响应对第一次失败使用 `max(retry_after, base_cooldown)`。
...
除了内置协议，你还可以在 YAML 中定义自定义协议并放置在 `~/.aleph/protocols/` 中。系统会监视此目录并自动重新加载定义。

### 工作原理

1. **File watcher** 检测 Create / Modify / Delete 事件（500ms 消抖）
2. **YAML parsed** 到 `ProtocolDefinition`
3. **`ConfigurableProtocol` adapter** 创建 —— 支持两种模式：
   - **Minimal mode** (`extends: openai`)：复用基础协议，覆盖 auth/header 差异
   - **Custom mode** (`custom:` block)：完整模板渲染，带 JSONPath 响应映射
4. **Registry updated** 通过 `ProtocolRegistry::register()` 原子更新
5. **New requests** 立即使用更新后的协议
...
**重载延迟：** 从文件变更到激活约 600ms（500ms 消抖 + 解析/注册开销）。
...
编辑 `src/providers/presets.rs` 并添加条目到 `PRESETS` HashMap：
...
- [网关](./gateway) — Gateway 如何将请求路由到提供商
- [Thinker](./thinker) — Thinker 如何选择和编排提供商
