...
MCP（模型上下文协议）方法管理外部 MCP 服务器 —— 添加、更新、删除、启动、停止和查询通过 MCP 协议暴露工具、资源和提示的服务器。
...
| `command` | string | 否 | 要运行的命令（stdio 用） |
| `args` | string[] | 否 | 命令参数 |
| `url` | string | 否 | HTTP URL（http 传输用） |
| `env` | object | 否 | 环境变量 |
| `requires_runtime` | string | 否 | 运行时要求（例如，`"node"`） |

### mcp.update

更新现有 MCP 服务器配置（使用 upsert 语义）。
